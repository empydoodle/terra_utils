  ...
  module Setup
    ...
    module ExeFileHelpers
      def rename_exe(path:, ind: 3)
        ...
        raise ArgumentError, 'Executable name cannot be blank!' if exe_name.empty?

        new_path = File.join(File.dirname(path), exe_name)
        raise ArgumentError, "File already exists! (#{new_path})" if File.exist?(new_path)
      ...
      def handle_existing_exe_symlink(path:, ind: 3)
        ...
        when 2
          # Rename
          rename_exe(path: path, ind: ind + 1)
        when 3
          # Do nothing
          ...
          # Error + retry
          raise ArgumentError, "Invalid option selected: #{option} - please select 1, 2 or 3"
        end
      rescue ArgumentError => e
        error_(e.message, ind)
        retry
      ...
      def resolve_exe_symlink(src_path, exe_path, ind: 2)
        ...
        if File.symlink?(exe_path)
          ...
          raise(ArgumentError, "Intended path for symlink already in use (#{exe_path} => #{File.readlink(exe_path)})")
        end
      ...
      def setup_exe_symlink(src_exe:, default_exe_name:, dir:, ind: 1)
        src_path = File.join(T_U_BIN_DIR, src_exe)
